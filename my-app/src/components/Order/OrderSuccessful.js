import React, { useState } from 'react'
import './OrderSuccessful.css'
import { Link } from 'react-router-dom';
// import { useRecoilState } from 'recoil'
// import { orderSuccessfulProvider } from '../../Providers/OrderSuccessfulProvider'

const OrderSuccessful = ({ orderid, message, redirecto }) => {

    // const [ordersuccesscont, setordersuccesscont] = useRecoilState(orderSuccessfulProvider)
    const [orderdata, setorderdata] = useState({
        OrderNo: orderid,
        OrderDate: '12/12/2021',
        OrderStatus: 'تم التسليم',
        CustomerName: 'محمد علي',
        CustomerShipToAddress: 'شارع الثلاثين، صنعاء، اليمن',
        CustomerEmail: 'mohammed.ali@example.com',
        OrderItems: [
            {
                ProductName: 'المنتج 1',
                Price: 150,
                Quantity: 3,
            },
            {
                ProductName: 'المنتج 2',
                Price: 200,
                Quantity: 2,
            }
        ],
        SubTotal: 700,
        Tax: 30,
        ShippingCharges: 50,
        Total: 780,
        PaymentType: 'الدفع عند الاستلام'
    })
    
    return (
        <>
        <div
            className='OrdersSuccessful'
            style={{color:"var(--color-darkest)",textAlign: "right" ,marginBottom:"24px",borderRadius:"12px",padding:"24px"
    }} dir="rtl"
        >
            <Link to={redirecto === 'userorders' ? '/user/yourorders' : '/yourorders'}>
                <button className='popup__close-btn'>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            </Link>

            <div className='confirmationcont'>
                <div className='c1'>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 01-1.043 3.296 3.745 3.745 0 01-3.296 1.043A3.745 3.745 0 0112 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 01-3.296-1.043 3.745 3.745 0 01-1.043-3.296A3.745 3.745 0 013 12c0-1.268.63-2.39 1.593-3.068a3.745 3.745 0 011.043-3.296 3.746 3.746 0 013.296-1.043A3.746 3.746 0 0112 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 013.296 1.043 3.746 3.746 0 011.043 3.296A3.745 3.745 0 0121 12z" />
                    </svg>
                    <h2>{message}</h2>
                </div>

                <div className='c2'>
                    <h2>ملخص الطلب</h2>
                    <div>
                        <p>رقم الطلب</p>
                        <p>{orderdata?.OrderNo}</p>
                    </div>
                    <div>
                        <p>تاريخ الطلب</p>
                        <p>{
                            orderdata.OrderDate
                        }</p>
                    </div>

                    <div>
                        <p>الاسم</p>
                        <p>{
                            orderdata.CustomerName
                        }</p>
                    </div>

                    <div>
                        <p>الإيميل</p>
                        <p>
                            {
                                orderdata.CustomerEmail
                            }
                        </p>
                    </div>

                    <div>
                        <p>المجموع الفرعي لطلب</p>
                        <p>$ {orderdata.SubTotal}</p>
                    </div>

                    <div>
                        <p>طريقة الدفع او السداد</p>
                        <p>{orderdata.PaymentType}</p>
                    </div>

                    <div>
                        <p>عنوان الشحن</p>
                        <p>{orderdata.CustomerShipToAddress
                        }</p>
                    </div>

                    <div>
                        <p>رسوم الشحن</p>
                        <p>$ {orderdata.ShippingCharges}</p>
                    </div>

                    <div>
                        <p>الضريبة</p>
                        <p>$ {orderdata.Tax}</p>
                    </div>

                    <div>
                        <p>المجموع</p>
                        <p>$ {orderdata.Total}</p>
                    </div>

                </div>

                <div className='c3'>
                    <table>
                        <thead>
                            <tr>
                                <th>رقم التسلسل</th>
                                <th>المنتج</th>
                                <th>السعر</th>
                                <th>الكمية</th>
                                <th>السعر الكلي</th>
                            </tr>
                        </thead>

                        <tbody>
                            {
                                orderdata?.OrderItems && orderdata.OrderItems.map((item, index) => {
                                    return (
                                        <tr key={index}>
                                            <td>
                                                <p>{index + 1}</p>
                                            </td>
                                            <td><p>{item.ProductName}</p></td>
                                            <td><p>${item.Price}</p></td>
                                            <td><p>{item.Quantity}</p></td>
                                            <td><p>${item.Price * item.Quantity}</p></td>
                                        </tr>
                                    )
                                })
                            }
                        </tbody>
                    </table>
                </div>

                <div className='totalcont'>
                    <div>
                        <p>المجموع الفرعي</p>
                        <p>$ {orderdata.SubTotal}</p>
                    </div>

                    <div>
                        <p>الشحن</p>
                        <p>$ {orderdata.ShippingCharges}</p>
                    </div>

                    <div>
                        <p>الضريبة</p>
                        <p>$ {orderdata.Tax}</p>
                    </div>

                    <div>
                        <p>المجموع</p>
                        <p>$ {orderdata.Total}</p>
                    </div>
                </div>
            </div>
        </div>
        </>
    )
}

export default OrderSuccessful